...
/** تنسيق رقم עוסק/ח.פ — نخفيه لو غير موجود */
function formatVatId(raw) {
  if (!raw) return ""; // إخفاء السطر كليًا
  // لو المستخدم كتب نص جاهز في الإعدادات نعرضه كما هو
  const v = String(raw).trim();
  const hasHeb = /[\u0590-\u05FF]/.test(v);
  if (hasHeb) return v;
  // غير هيك نعرضه بعنوان واضح
...
async function loadCompanyHeader() {
  ...
  return {
    ...
    vatId: formatVatId(map.company_vat || ""), // سيعود "" لو غير موجود → لن نعرضه
...
/* =========================
   عناصر الواجهة (Layout)
   ========================= */

// شريط علوي ملوّن مع عنوان المستند
...
// صندوق بيانات الشركة (يمين) والعميل (يسار)
...
function drawItemsTable(doc, columns, rows) {
  ...
  const colGap = 20; // المسافة بين الأعمدة

  // Header
  doc.save();
  doc.rect(startX, doc.y, tableWidth, 24).fill(headerBg);
  let x = startX;
  const headerY = doc.y + 6; // نفس Y لكل الأعمدة
  ...
  rows.forEach(r => {
    ...
    doc.y += rowHeight; // ننتقل للسطر التالي
  });
}
...
// مربع المجاميع (قبل־מע״מ / מע״מ / סה״כ)
...
router.get("/order/:orderId.pdf", async (req, res) => {
  ...
  try {
    ...
    // آخر دفعة مرتبطة بالطلب (قد تحتوي TransactionID)
    ...
    // في حال عدم وجود دفعة، خذ إجمالي الطلب
...
async function buildSubscriptionReceipt(res, { userId = null, subscriptionId = null, paymentId = null }) {
  ...
  if (!payment && subscriptionId) {
    ...
    if (pByRef) {
      ...
      // أحدث دفعة اشتراك لنفس المستخدم
  ...
  // نوع الاشتراك ووسيلة الحامل (من جدول الاشتراك)
...
const columns = [
  ...
  // فوتر
  doc.fontSize(THEME.fontSize.small).fillColor("#666")
     .text("המסמך הופק אוטומטית ממערכת FitTrack. אין צורך בחתימה.", { align: "center" });

  doc.end();
}

/* =========================
   مسارات كبلות الاشتراك
   ========================= */
