...
// ======================= (جديد) الطلبيات الجديدة للتنبيهات =======================
// ✅ رجّعنا Status كمان عشان البوب־أب يعرف إنها Pending ويظهر زر "התקבלה"
router.get('/admin/new-orders', async (req, res) => {
  try {
    const [orders] = await db.query(
      `SELECT o.OrderID, o.OrderDate, o.TotalPrice, o.Status, u.Name, u.Email, u.Phone
       FROM orders o
       JOIN users u ON o.UserID = u.UserID
    ...
    res.status(500).json({ error: "שגיאה בקבלת הזמנות חדשות", details: err });
  }
});

// ======================= (اختياري) تفاصيل طلب واحد للإدارة =======================
// يفيد لو حبيتي دائمًا تجيبي الحالة الأحدث قبل فتح البوب־أب
router.get('/admin/:orderId', async (req, res) => {
  const { orderId } = req.params;
  try {
    const [rows] = await db.query(
      `SELECT o.OrderID, o.OrderDate, o.Status, o.TotalPrice, u.Name, u.Email, u.Phone
       FROM orders o
       JOIN users u ON o.UserID = u.UserID
       WHERE o.OrderID = ?`,
      [orderId]
    );
    if (rows.length === 0) return res.status(404).json({ error: "ההזמנה לא נמצאה" });
    res.json(rows[0]);
  } catch (err) {
    res.status(500).json({ error: "שגיאה בקבלת הזמנה", details: err });
  }
});
...
// ======================= ביטול הזמנה (אם لا تزال Pending) =======================
