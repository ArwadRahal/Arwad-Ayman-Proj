...
// מחזיר את כל המפגשים עם פרטי המאמן
...
// יוצר מפגש חדש במערכת
router.post("/", async (req, res) => {
  const {
    trainer_id,      // מזהה המאמן
    session_type,    // סוג המפגש
    session_date,    // תאריך המפגש
    start_time,      // שעת התחלה
    end_time         // שעת סיום (אופציונלי)
  } = req.body;
...
// מחזיר את כל המפגשים עם סטטוס הרשמה למתאמנת נתונה
...
// מחזיר את כל המפגשים שאליהם מתאמנת מסוימת רשומה
...
// מחזיר את רשימת השמות של כל המשתתפים במפגש
...
// רושם מתאמנת למפגש (עם IGNORE למניעת כפילויות)
...
// מוחק מפגש וכל הרשומות של המשתתפים בו
