...
export default function CalendarView({ refreshFlag }) {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      ...
      const mapped = rawList.map((item) => {
        ...
        const exerciseTypeObj = typesList.find(
          (t) =>
            t.ExerciseTypeID ===
        ...
        const startLocal = parseLocalDateTime(session_date, start_time);
        ...
        return {
          ...
          id:
            (item.id !== undefined && item.id !== null
              ...
              : item.ExerciseID) + "",
          title:
            (TYPE_LABELS[type] || type) + (typeName ? ` (${typeName})` : ""),
          type,
      ...
      if (user.role === "Trainee") {
        ...
        try {
          const subRes = await fetch(
            `http://localhost:8801/subscription/active/${user.id}`
          );
          const subData = await subRes.json();
          membership =
            subData?.sub?.MembershipType ||
            subData?.MembershipType ||
            subData?.membershipType ||
            null;
        } catch {
          membership = null;
        ...
        const filtered = mapped.filter((ev) => {
          const isReg = Number(ev.is_registered) === 1;
          const future = ev._startLocal
            ? ev._startLocal.getTime() > now.getTime()
            : false;
          if (isReg) return true;
          if (!future) return false;
          if (!membership) return false;
          return ev.allowed_membership === membership;
        });
  ...
  const handleEventClick = async (info) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8801/schedule/participants/${info.event.id}`
      );
  ...
  const handleRegister = async () => {
    if (selectedEvent?._startLocal && selectedEvent._startLocal <= new Date()) {
      setErrorMsg("לא ניתן להירשם לאימון שעבר.");
      return;
    }
    try {
      setErrorMsg("");
  ...
  // إضافة تمرين — للـ Admin فقط
  const handleAddSubmit = async (data) => {
    if (role !== "Admin") {
      alert("רק מנהלת יכולה להוסיף אימון.");
      return;
    }
    try {
  ...
  function canUnregisterFunc(event) {
    if (!event || !event._startLocal) return false;
    const now = new Date();
    const diffHours = (event._startLocal - now) / (1000 * 60 * 60);
    return diffHours >= 4;
  }
    ...
    return (
      <div className="calendar-legend" style={{ display: "flex", gap: "2em", marginBottom: 10, fontSize: 18 }}>
        <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <span style={{ display: "inline-block", width: 14, height: 14, borderRadius: "50%", background: TYPE_COLORS.Group, marginInlineEnd: 6, border: "1.5px solid #888" }}></span>
          קבוצתי
        </span>
        <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <span style={{ display: "inline-block", width: 14, height: 14, borderRadius: "50%", background: TYPE_COLORS.Couple, marginInlineEnd: 6, border: "1.5px solid #888" }}></span>
          זוגי
        </span>
        <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <span style={{ display: "inline-block", width: 14, height: 14, borderRadius: "50%", background: TYPE_COLORS.Single, marginInlineEnd: 6, border: "1.5px solid #888" }}></span>
          אישי
        </span>
      </div>
    );
    ...
    <div className="calendar-wrapper">
      {(role === "Admin" || role === "Coach") && (
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          {renderLegend()}
          {/* زر الإضافة يظهر للـ Admin فقط */}
          {role === "Admin" && (
            <div className="add-btns-row">
              <button
                className="add-btn big"
                onClick={async () => {
                  if (role !== "Admin") return; // حارس إضافي
                  try {
                    const res = await fetch("http://localhost:8801/users?role=Coach");
                    const data = await res.json();
                    setCoaches(Array.isArray(data) ? data : []);
      ...
      <EventModal
        ...
        actions={
          !selectedEvent
            ...
            : role === "Trainee" &&
              selectedEvent.is_registered !== 1 &&
              selectedEvent._startLocal &&
              selectedEvent._startLocal > new Date()
            ? (
        ...
        eventTypeColor={selectedEvent ? TYPE_COLORS[selectedEvent.type] : undefined}
      >
      ...
      {/* نموذج الإضافة متاح فقط للـ Admin */}
      {role === "Admin" && addEvent && (
        <AddOrEditEventForm
          event={null}
...
function renderEventContent(arg) {
  ...
  const role = JSON.parse(localStorage.getItem("currentUser"))?.role || "Trainee";
  ...
  return (
    ...
    >
      ...
      {arg.event.extendedProps.session_type && (
        <span style={{ color: "#444", fontWeight: 400, fontSize: 13, marginInlineStart: 4 }}>
          ({arg.event.extendedProps.session_type})
        </span>
      )}
      <br />
      {startTime && endTime && (
        <span style={{ color: "#333", fontSize: 13 }}>
          <span style={{ fontWeight: 500 }}>{startTime}–{endTime}</span>
