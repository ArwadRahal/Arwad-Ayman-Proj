...
export default function StatusPage() {
  ...
  function FinanceSummary() {
    if (loadingFinance)
      return <div className="finance-summary">טוען נתונים פיננסיים...</div>;
    if (!finance)
      return (
        <div className="finance-summary" style={{ color: "#c00" }}>
          שגיאה בטעינת מידע פיננסי
        </div>
      );
  ...
  return (
    <div className="status-dashboard">
      ...
      {/* الرسم البياني: أسماء المدربات ثابتة على اليسار */}
      <div
        style={{
          background: "#fff",
          borderRadius: 16,
          padding: 16,
          boxShadow: "0 6px 18px #00000008",
          margin: "0 auto 24px",
          width: "min(880px, 95%)",
          direction: "ltr", // مهم حتى لا يختفي محور Y في RTL
        }}
      >
        <h3 style={{ textAlign: "center", marginBottom: 6 }}>
          המאמנות – מדד פופולריות (חודש נוכחי)
        </h3>
        <div style={{ textAlign: "center", color: "#777", marginBottom: 12, fontSize: 14 }}>
          מספר אימונים לכל מאמנת
        </div>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <BarChart
            data={coaches.map((c) => ({
              name: c?.Name || "ללא שם",
              value: Number(c?.SessionsCount || 0),
            }))}
            layout="vertical"
            margin={{ top: 8, right: 24, bottom: 8, left: 140 }} // مساحة كافية للأسماء
          >
            ...
            />
            <Tooltip
              formatter={(val) => [`${val}`, "כמות אימונים"]}
              labelFormatter={(label) => `מאמנת: ${label}`}
            />
            <Bar dataKey="value" fill="#2ecc71" radius={[6, 6, 6, 6]}>
      ...
      <div className="stats-cards">
        ...
        <div className="stat-box unpaid">
          ...
          <p>{
            trainees.filter((t) => t.status === "Expired" || t.status === "NotPaid").length
          }</p>
        </div>
      </div>
      ...
      <table className="trainee-table coach-table">
        ...
        <tbody>
          {coaches.length === 0 ? (
            <tr><td colSpan={4}>אין מאמנות להצגה</td></tr>
          ) : (
            coaches.map((c) => (
              <tr key={c.UserID}>
                <td>
