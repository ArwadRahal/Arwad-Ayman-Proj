...
export default function TraineeView() {
  ...
  function calcAmount() {
    ...
    return payType === 'Monthly' ? 250 : 3600; 
  }
        ...
        <div className="popup-box" style={{ maxWidth: 420, marginTop: 120 }}>
          ...
          <div className="popup-content">
            ...
            <div className="paypal-wrap" style={{ opacity: readyForPayPal ? 1 : 0.5 }}>
              {readyForPayPal ? <PayPalButtonInline amount={amount} onApprove={handleApproveSubscription} /> :
                <button style={{ width:'100%', padding:'10px 0' }} disabled>בחרי סוג מנוי ותקופה להצגת תשלום PayPal</button>}
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="trainee-dashboard">
      ...
      <div className="week-nav-row">
        <button className="week-nav-btn" onClick={() => setStartOfWeek(prev => { const d = new Date(prev); d.setDate(prev.getDate() - 7); return d; })}>{'← שבוע קודם'}</button>
        ...
        <button className="week-nav-btn" onClick={() => setStartOfWeek(prev => { const d = new Date(prev); d.setDate(prev.getDate() + 7); return d; })}>{'שבוע הבא →'}</button>
      </div>
