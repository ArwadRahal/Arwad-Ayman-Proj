...
function PayPalButtonInline({ amount, onApprove }) {
  ...
  useEffect(() => {
    ...
    return () => { try { buttons && buttons.close(); } catch {} };
  }, [amount, onApprove]);

  return <div ref={containerRef} />;
}

function ManagerPublicProfileModal({ open, onClose, manager }) {
  if (!open || !manager) return null;

  const avatar = manager.imageURL
    ? `http://localhost:8801/uploads/${manager.imageURL}`
    : defaultAvatar;

  const ig =
    manager?.social?.instagram ||
    manager?.social?.ig ||
    manager?.social?.instagramUrl ||
    manager?.instagram ||
    null;
  ...
  return (
    <div className="popup-overlay" onClick={onClose}>
      <div
        className="popup-box aup-wrap"
        onClick={(e) => e.stopPropagation()}
        role="dialog" aria-modal="true"
      >
        <div className="popup-header">
          <h2 style={{margin:0}}>פרופיל המנהלת</h2>
          <button
            onClick={onClose}
        ...
        <div className="popup-content">
          <div className="aup-head">
            <img
              ...
              className="aup-avatar-lg"
            />
            <div className="aup-title">{manager.name || 'מנהלת'}</div>
            <div className="aup-sub">מאמנת ראשית</div>
          </div>

          <div className="aup-grid-2">
            <div className="aup-field">
              <div className="aup-label">שם מלא</div>
              <div className="aup-value">{manager.name || '—'}</div>
            </div>
            <div className="aup-field">
              <div className="aup-label">אימייל</div>
              <div className="aup-value">{manager.email || '—'}</div>
            </div>
            <div className="aup-field">
              <div className="aup-label">טלפון</div>
              <div className="aup-value">{manager.phone || '—'}</div>
            </div>
            <div className="aup-field">
              <div className="aup-label">אינסטגרם</div>
              <div className="aup-value">
                {ig ? (
                  <a className="aup-link" href={ig} target="_blank" rel="noreferrer">
                    עקבי באינסטגרם
                  </a>
                ) : '—'}
              </div>
          ...
          <div className="aup-footer-note">תצוגה לקריאה בלבד</div>
        </div>
      </div>
    </div>
  );
...
export default function TraineeView() {
  ...
  useEffect(() => {
    fetch('http://localhost:8801/settings/vat')
      .then(r => r.json())
      .then(d => setVatPercent(Number(d?.vat) || 0))
  ...
  // عدد الجلسات المنفّذة هذا الشهر
  useEffect(() => {
    if (!traineeId) return;
  ...
  // بيانات المعلّمة/المديرة
  useEffect(() => {
    fetch('http://localhost:8801/admin/manager-public')
      .then(res => res.json())
  ...
  function calcAmountWithVat() {
    const base = calcAmount();
    const total = +(base * (1 + (vatPercent || 0) / 100)).toFixed(2);
  ...
  async function handleApproveSubscription(details) {
    ...
    try {
      ...
      // 2) تسجيل الدفعة (نرسل الأساس والسيرفر سيضيف الضريبة)
      try {
        await fetch('http://localhost:8801/payments', {
          ...
          body: JSON.stringify({
            userId: traineeId,
            amount: Number(baseAmount),                 // أساس
            paymentFor: 'Subscription',
            referenceId: addData?.subscriptionId || null, // رقم الاشتراك
            transactionId: details?.id || null,        // من PayPal
            status: 'Completed'
          })
        });
  ...
  if (!subscription.active || showPayPopup) {
    const baseAmount = calcAmount();
    const totalWithVat = calcAmountWithVat();
    const readyForPayPal = totalWithVat > 0;
      ...
      <div className="popup-overlay">
        <div className="popup-box" style={{ maxWidth: 420, marginTop: 120 }}>
          <div className="popup-header">
            ...
            <button onClick={() => setShowPayPopup(false)} style={{fontSize:22,border:'none',background:'none'}} aria-label="close">×</button>
          </div>
          <div className="popup-content">
            <div style={{ marginBottom: 14, color:'#999' }}>יש להשלים תשלום מנוי כדי לגשת לאימונים.</div>

            <select style={{ width:'100%', marginBottom:10 }} value={membershipType} onChange={e => setMembershipType(e.target.value)} required>
            ...
            <select style={{ width:'100%', marginBottom:10 }} value={payType} onChange={e => setPayType(e.target.value)} required>
            ...
            <div style={{ background:'#faf7fd', border:'1px solid #eee', borderRadius:10, padding:10, marginBottom:10 }}>
              סכום לפני מע״מ: <b>{baseAmount || 0} ₪</b><br/>
              מע״מ {vatPercent || 0}%: <b>{+((baseAmount||0) * (vatPercent/100)).toFixed(2)} ₪</b><br/>
              <span style={{fontWeight:800}}>לתשלום (כולל מע״מ): {totalWithVat || 0} ₪</span>
            </div>
            ...
            <div className="paypal-wrap" style={{ opacity: readyForPayPal ? 1 : 0.5 }}>
              {readyForPayPal ? (
                <PayPalButtonInline amount={totalWithVat} onApprove={handleApproveSubscription} />
              ) : (
  ...
  // ====== إعدادات حد الجلسات الشهري ======
  const MONTHLY_LIMIT = 12;
  const meetingsUsed = monthlyMeetings || 0;
  const meetingsLeft = Math.max(0, MONTHLY_LIMIT - meetingsUsed);
    ...
    <div className="trainee-dashboard">
      <div className="trainee-header-row">
        ...
        <div className="header-right-stack">
          ...
          {manager && (
            <button
              className="manager-inline-card"
              onClick={() => setMgrOpen(true)}
              title="פרופיל המנהלת"
            >
              <img
                src={manager.imageURL ? `http://localhost:8801/uploads/${manager.imageURL}` : defaultAvatar}
                onError={(e)=>{ e.currentTarget.src = defaultAvatar; }}
                alt={manager.name || 'manager'}
                className="manager-inline-avatar"
              />
              <span className="manager-inline-text">פרופיל המנהלת</span>
            </button>
          )}
        </div>
      </div>

      {/* المتبقّي من أصل العدد الكُلّي + المنجَز */}
      <div className="monthly-meetings-row">
        {meetingsUsed < MONTHLY_LIMIT ? (
          <>
            נותרו לך <b>{meetingsLeft}</b> מתוך <b>{MONTHLY_LIMIT}</b> מפגשים החודש
            <span style={{ marginInlineStart: 10, color: '#7b6bbd' }}>
              (בוצעו: {meetingsUsed})
            </span>
          </>
        ) : (
          <>הגעת למספר המפגשים החודשי המותר ({MONTHLY_LIMIT})</>
        )}
      </div>
      ...
      <div className="week-nav-row">
        <button
          ...
          onClick={() =>
            setStartOfWeek(prev => { const d = new Date(prev); d.setDate(prev.getDate() - 7); return d; })
          }
        >
          {'← שבוע קודם'}
        </button>
