  ...
  const canEdit = currentUser?.id === user.id || currentUser?.role === "Admin";
   ...
   function SubscriptionModal() {
    ...
    const daysLeft = Math.max(
      0,
      Math.ceil((new Date(subDetails.sub.EndDate) - new Date()) / (1000 * 60 * 60 * 24))
    );

 const receiptHref = `${API}/receipts/subscription/${subDetails.sub.SubscriptionID}.pdf`;
    ...
    return (
      <div className="modal-overlay">
        <div className="modal-content" style={{ maxWidth: 460 }}>
          ...
          {/* زر تنزيل الكבלה داخل البوب־أب */}
          <a
            href={receiptHref}
            target="_blank"
            rel="noopener noreferrer"
            className="save-btn"
            style={{ textAlign: "center", marginTop: 14, textDecoration: "none" }}
          >
            הורדת קבלה
          </a>
