...
export default function UsersStatusPage() {
  ...
  const [statusFilter, setStatusFilter] = useState("all"); // all | Active | ExpiringSoon | ExpiredOrNotPaid
  const [typeFilter, setTypeFilter] = useState("all");     // all | Monthly | Yearly
  const [sortBy, setSortBy] = useState("status");          // status | endDate | name
  ...
  const renderAvatar = (user) =>
    user.ImageURL ? (
      <img src={`${API}/uploads/${user.ImageURL}`} alt={user.Name} />
    ) : (
      <img
        src={defaultAvatar}
        alt="avatar"
  ...
  const redCount   = trainees.filter((t) => t.status === "Expired" || t.status === "NotPaid").length;

  // فلترة وترتيب جدول المتدرّبات
  const filtered = useMemo(() => {
    ...
    if (statusFilter !== "all") {
      if (statusFilter === "ExpiredOrNotPaid") {
        list = list.filter((t) => t.status === "Expired" || t.status === "NotPaid");
      } else {
    ...
    if (typeFilter !== "all") {
      list = list.filter(
        (t) => (t.SubscriptionType || "").toLowerCase() === typeFilter.toLowerCase()
      );
    }
  ...
  // بيانات الرسم: أسماء المدرّبات + عدد المتدرّبات المختلفات
  const coachBarData = coaches.map((coach) => ({
    name: coach?.Name || "ללא שם",
    trainees: Number(coach?.UniqueTraineesCount ?? 0),
  }));

  return (
    <div className="status-dashboard">
      <h2 style={{ marginTop: 24 }}>המאמנות</h2>

      <div className="chart-card">
        <h3 className="chart-title">המאמנות – מדד פופולריות (חודש נוכחי)</h3>
        <div className="chart-sub">מספר מתאמנות שונות לכל מאמנת</div>
          ...
          {coachBarData.length === 0 ? (
            ...
            >
              ...
              <Label 
                ...
                position="bottom"    
                offset={10}     
                style={{ fill: "#6b47b6", fontWeight: 600, fontSize: 13 }}
              />
            </XAxis>
              ...
              >
                ...
                <LabelList dataKey="trainees" position="right" />
              </Bar>
              ...
              <tr key={c.UserID}>
                ...
                <td style={{ textAlign: "center" }}>
                  <span
                    style={{
                      fontWeight: 700,
                      color: (c.UniqueTraineesCount ?? 0) >= 6 ? "#14b181" : "#e2764a",
                    }}
                  >
      ...
      <div className="filters-bar">
        <div className="pills-wrap">
          <div className="pills" role="tablist" aria-label="Filter by subscription type">
            <button
      ...
      <div className="stats-cards">
        <button
          className={`stat-box paid ${statusFilter === "Active" ? "active" : ""}`}
        ...
        <button
          className={`stat-box expiring ${statusFilter === "ExpiringSoon" ? "active" : ""}`}
          onClick={() =>
            setStatusFilter(statusFilter === "ExpiringSoon" ? "all" : "ExpiringSoon")
          }
        ...
        <button
          className={`stat-box unpaid ${statusFilter === "ExpiredOrNotPaid" ? "active" : ""}`}
          onClick={() =>
            setStatusFilter(
              statusFilter === "ExpiredOrNotPaid" ? "all" : "ExpiredOrNotPaid"
            )
          }
            ...
            filtered.map((t) => {
              ...
              return (
                <tr key={t.UserID}>
                  ...
                  <td style={{ textAlign: "center" }}>{t.SubscriptionType || "-"}</td>
                  <td style={{ textAlign: "center" }}>
