...
export default function MyOrders() {
  ...
  // قسم الاشتراك
  const [subInfo, setSubInfo] = useState(null); // { payment, subscription, breakdown }

  useEffect(() => {
    if (!userId) return;
    fetchOrders();
    fetch(`${API}/payments/subscription/latest/${userId}`)
      .then(r => r.json())
      .then(setSubInfo)
  ...
  const toNis = (n) => new Intl.NumberFormat("he-IL", { style: "currency", currency: "ILS" }).format(+n || 0);

  return (
    <div className="payments-history">
      <h2 style={{textAlign:"center", color:"#845ec2"}}>ההזמנות שלי</h2>

      {/* قسم الاشتراك أعلى الصفحة */}
      {subInfo && subInfo.subscription && subInfo.payment && (
        <div className="sub-receipt-card">
          <div className="sub-receipt-col">
            <div className="sub-title">מנוי פעיל</div>
            <div className="sub-line">
              סוג: <b>{subInfo.subscription.Type === "Yearly" ? "שנתי" : "חודשי"}</b>
            </div>
            <div className="sub-line">
              תוקף: <b>
                {new Date(subInfo.subscription.StartDate).toLocaleDateString("he-IL")} — {new Date(subInfo.subscription.EndDate).toLocaleDateString("he-IL")}
              </b>
            </div>
          </div>
          <div className="sub-receipt-col">
            <div className="sub-title">תשלום אחרון</div>
            <div className="sub-line">מס׳ תשלום: <b>{subInfo.payment.PaymentID}</b></div>
            <div className="sub-line">תאריך: <b>{new Date(subInfo.payment.PaymentDate).toLocaleString("he-IL")}</b></div>
            <div className="sub-line">סה״כ: <b>{toNis(subInfo.breakdown?.totalAmount)}</b></div>
          </div>
          <div className="sub-receipt-actions">
            <button
              className="actionBtn viewBtn"
              onClick={() => window.open(`${API}/receipts/subscription/latest/${userId}.pdf`, "_blank")}
            >
              הורד קבלה (PDF)
            </button>
          </div>
          ...
          <div className="popup-box">
            ...
            {payment && (
              <div className="receipt-box">
                <h4 className="receipt-title">חשבונית מס / קבלה</h4>
                <table>
                  <tbody>
                    <tr><td>מס׳ תשלום:</td><td>{payment.PaymentID}</td></tr>
                    <tr><td>סכום לתשלום:</td><td>{toNis(payment.Amount)}</td></tr>
                ...
                {payment.Status === "Completed" && (
                  <div style={{ marginTop: 12, textAlign: "center" }}>
                    <button
                      className="actionBtn viewBtn"
                      onClick={() => window.open(`${API}/receipts/order/${selectedOrder.OrderID}.pdf`, "_blank")}
                    >
                      הורד קבלה (PDF)
                    </button>
                  </div>
