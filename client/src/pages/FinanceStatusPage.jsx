...
function deriveFinance(finance) {
  ...
  if (Array.isArray(raw) && raw.length) {
    const valid = raw.filter((s) => !s.Canceled && (s.Attendees ?? 0) > 0);
...
export default function FinanceStatusPage() {
  ...
  async function handleSaveVat() {
    ...
    try {
      setSavingVat(true);
      const res = await axios.put(`${API}/settings/vat`, { vat: n });
      if (res.data?.success) {
        setCurrentVat(n);
        setVatModalOpen(false);
        // بإمكانك إعادة تحميل البيانات المالية إن أردت:
        // await fetchFinance();
      } else {
