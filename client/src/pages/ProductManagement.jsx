...
export default function ProductManagement() {
  ...
  useEffect(() => {
    fetch(`${API}/settings/vat`)
      .then(res => res.json())
      .then(data => {
        if (typeof data.vat === 'number') setVatPercent(data.vat);
      })
      .catch(() => {});
  }, []);
  ...
  async function handleSaveProduct() {
    ...
    try {
      if (!editProductCode) {
        ...
        await axios.put(`${API}/products/${editProductCode}`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
  ...
  async function handleDelete(productCode, name) {
    if (!window.confirm(`האם למחוק את המוצר "${name}" לצמיתות?`)) return;
    try {
  ...
  async function handleEditCategory() {
    ...
    try {
      await axios.put(`${API}/categories/${editCat.CategoryID}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
  ...
  const filteredProducts = selectedCategory
    ...
    : products;
  const popupPriceWithVat = useMemo(() => {
    const p = Number(newProduct.Price);
    if (!p || isNaN(p) || p <= 0) return '';
    return (Math.round(p * (1 + vatPercent / 100) * 100) / 100).toFixed(2);
  }, [newProduct.Price, vatPercent]);
  return (
    <div className="product-management-cards">
      <div className="product-management-header">
        <h2>ניהול מוצרים</h2>

        <div style={{ marginInlineStart: 'auto', marginInlineEnd: 12, fontWeight: 600, color: '#7a5db4' }}>
          מע״מ נוכחי: <span style={{ background:'#efe8ff', padding:'4px 8px', borderRadius:8 }}>
            {typeof vatPercent === 'number' ? `${vatPercent}%` : '—'}
          </span>
        </div>
        ...
        <div className="popup-overlay">
          <div className="popup-box popup-box-category">
            ...
            {editCatImage ? (
              ...
              <img
                src={`${API}/uploads/${editCat.ImageURL}`}
        ...
        <>
          ...
          <div className="product-cards-list">
            {filteredProducts.map((prod) => {
              const priceWithVat =
                Math.round((Number(prod.Price) * (1 + vatPercent / 100)) * 100) / 100;
              return (
                <div className="product-card" key={prod.ProductCode}>
                  <img
                    className="product-image"
                    src={`${API}/uploads/${prod.ImageURL}`}
                    alt={prod.Name}
                  />
                  <div className="product-info">
                    ...
                    <p>
                      <b>סוג:</b>{" "}
                      {categories.find(c => c.CategoryID === prod.CategoryID)?.CategoryName || ""}
                    </p>
                    ...
                    <p style={{ marginTop: -8 }}>
                      <b>מחיר לצרכן:</b>{" "}
                      <span className="price" title="כולל מע״מ">
                        {priceWithVat.toFixed(2)} ₪
                      </span>{" "}
                      <span style={{ color: '#888', fontSize: 12 }}>(כולל מע״מ {vatPercent}%)</span>
                    </p>
      ...
      {showPopup && (
        <div className="popup-overlay">
          <div className="popup-box">
            ...
            >
              מחיר לצרכן (כולל מע״מ {vatPercent}%):{" "}
              <span>{popupPriceWithVat ? `${popupPriceWithVat} ₪` : '—'}</span>
            </div>
